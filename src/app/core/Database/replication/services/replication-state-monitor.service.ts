...
import { ReplicationManagerService } from './replication-manager.service';
import {
  Observable,
  combineLatest,
  of,
...
export class ReplicationStateMonitorService {
  ...
  constructor(private replicationManager: ReplicationManagerService) {
    // Initialize wasStarted tracking for all replications
    this.initializeWasStartedTracking();

    // Register this service with ReplicationManagerService to receive state change notifications
    this.replicationManager.setReplicationMonitorService(this);
  }
  ...
  private initializeWasStartedTracking(): void {
    const allStates = this.replicationManager.getAllReplicationStates();
  ...
  getReplicationState(identifier: string): ReplicationStateInfo | null {
    const state = this.replicationManager.getReplicationState(identifier);
    if (!state) {
      return null;
    }
  ...
  getAllReplicationsState(): AllReplicationsState {
    ...
    // Get all replication identifiers from constants
    const identifiers = REPLICATION_IDENTIFIERS;
  ...
  getCollectionReplicationReceived$(
    ...
    const primaryState = this.replicationManager.getReplicationState(
      mapping.primary,
    );
    const secondaryState = this.replicationManager.getReplicationState(
      mapping.secondary,
    );
